...
export const CompanyLogo: React.FC<{ name: string; logoUrl?: string; className?: string }> = ({ name, logoUrl, className = "w-9 h-9" }) => {
  ...
  if (brandKey) {
    ...
    borderClass = matchedStyle.border === 'border-transparent' 
      ? (matchedStyle.bg === 'bg-white' && isLight ? 'border-slate-300 shadow-xs' : 'border-transparent') 
      : (isLight ? 'border-slate-300 shadow-xs' : matchedStyle.border);
  } else if (logoUrl && !imageFailed) {
    bgClass = 'bg-white';
    borderClass = isLight ? 'border-slate-300 shadow-xs' : 'border-white/10';
  }

  return (
    <span data-company-logo="true" className={`${className} rounded-full flex items-center justify-center p-1 overflow-hidden flex-shrink-0 hover:scale-110 transition-all duration-200 border ${bgClass} ${borderClass}`}>
      {renderInnerLogo()}
    </span>
  );
};
