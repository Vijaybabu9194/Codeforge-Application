...
import { Code2, ArrowRight, Sun, Moon, Lock, Mail, User, Sparkles, CheckCircle2 } from 'lucide-react';
    ...
    <div className={`min-h-screen ${dark ? 'bg-[#050A18] text-white' : 'bg-[#F8FAFC] text-slate-900'} relative flex flex-col justify-between overflow-hidden select-none font-sans transition-colors duration-300`}>
      ...
      <header className="px-6 py-5 flex items-center justify-between relative z-20 max-w-7xl mx-auto w-full">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onSwitchToLogin} className="text-xs font-bold text-sky-600 hover:underline px-3 py-2">
        ...
        <div className={`w-full max-w-md rounded-2xl border ${dark ? 'border-slate-800 bg-slate-900/80' : 'border-slate-200 bg-white/90'} p-8 shadow-2xl backdrop-blur-xl relative overflow-hidden transition-all duration-300`}>
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-sky-500/10 text-sky-600 border border-sky-500/20 mb-3">
              <Sparkles className="w-3.5 h-3.5" /> Start Your 100% Free Account
            </div>
            <h1 className="text-2xl font-black tracking-tight mb-2">Create Your Developer Profile</h1>
            <p className="text-xs text-slate-500 dark:text-slate-400">Join 10,000+ developers practicing DSA and tracking interview readiness.</p>
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-800 space-y-2">
            <div className="flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 flex-shrink-0" /> Free forever access to 1,500+ DSA problems
            </div>
            <div className="flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 flex-shrink-0" /> Company-wise interview question analytics
            </div>
          </div>
        </div>
      </main>
